...
file = open("./data/data15k.csv", newline="")
temp = 0.0
...
def load_data():
    ...
    # normalizing data on loading instead on every iteration
    column_number = 2
    for i in range(column_number):
        col = [float(point[i]) for point in data]
        minumim_val = min(col)
        maximum_col = max(col)
        diff = maximum_col - minumim_val
        for point in data:
            point[i] = (float(point[i]) - minumim_val) / diff
